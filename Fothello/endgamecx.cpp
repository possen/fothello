...
20-30% worse than the low-level code in the program Zebra to which I am the
...
* Removed killer heuristics from the entire program as it hurt performance
  ...
  now ignored by the search code. (1% speedup)
...
PrepareToSolve( uchar *board )
{
   ...
   k = 1;
   for(i=10; i<=80; i++){
      if(board[i]==END_EMPTY){
         if( board[i-10]==END_EMPTY ) HoleId[i] = HoleId[i-10];
         else if( board[i-9]==END_EMPTY ) HoleId[i] = HoleId[i-9];
         else if( board[i-8]==END_EMPTY ) HoleId[i] = HoleId[i-8];
         else if( board[i-1]==END_EMPTY ) HoleId[i] = HoleId[i-1];
         else{ HoleId[i] = k; k<<=1; }
      }
      else HoleId[i] = 0;
   }
#define MAXITERS 1
   ...
   for(z=MAXITERS; z>0; z--){
      for(i=80; i>=10; i--){
         if(board[i]==END_EMPTY){
            k = HoleId[i];
            if( board[i+10]==END_EMPTY ) HoleId[i] = minu(k,HoleId[i+10]);
            if( board[i+9]==END_EMPTY ) HoleId[i] = minu(k,HoleId[i+9]);
            if( board[i+8]==END_EMPTY ) HoleId[i] = minu(k,HoleId[i+8]);
            if( board[i+1]==END_EMPTY ) HoleId[i] = minu(k,HoleId[i+1]);
         }
      }
      for(i=10; i<=80; i++){
         if(board[i]==END_EMPTY){
...
"wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwbwwwwwwwbwwwwbbb.",
   ...
   for(i=0; i<112; i++){
      ...
      for(j=0; j<64; j++){
         ...
         if(bds[i][j]=='w'){ board[k] = END_WHITE; wc++; }
         else if(bds[i][j]=='b'){ board[k] = END_BLACK; bc++; }
         else if(bds[i][j]=='.'){ board[k] = END_EMPTY; emp++; }
      }
      PrepareToSolve( board );
      for(tries=0; tries<MAXTRIES; tries++){
#if FULLSOLVE==2
         val = EndSolve( board, -64, 64, END_WHITE, emp, wc-bc, 1 );
#else
#if FULLSOLVE==1
         val = EndSolve( board, -1, 1, END_WHITE, emp, wc-bc, 1 );
#else
         val = EndSolve( board,  0, 1, END_WHITE, emp, wc-bc, 1 );
#endif
#endif
      }
