...
- (void)displayScore
{
    ...
    winLabel.position = CGPointMake(CGRectGetMidX(self.boardScene.frame),
                                    CGRectGetMidY(self.boardScene.frame));
...
- (void)present
{
    ...
    [self fadePiecesTo:.4];
    [self animatePlayer:player1 movePos:boardScene.frame.size.width / 4];
    [self animatePlayer:player2 movePos:boardScene.frame.size.width / 4 * 3];
}
